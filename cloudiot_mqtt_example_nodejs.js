...
const publishAsync = async (
  ...
  setTimeout(() => {
    ...
    const payload = Buffer.from(
      JSON.stringify({ cpu: cpuData.speed, memory: memoryData.active, temperature: cpuTemp.main, battery: batteryData.percent, time: time.current})
    );

    // Publish "payload" to the MQTT topic. qos=1 means at least once delivery.
    // Cloud IoT Core also supports qos=0 for at most once delivery.
    console.log("Publishing message:", JSON.parse(payload.toString()));
    payload &&
...
const listenForConfigMessages = (
  ...
  client.on("message", (topic, message) => {
     console.log("##meassage", JSON.stringify(message));
